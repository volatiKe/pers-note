...
* 业务：
	* 付款的实时清结算如 0 元购退款时想将资金退至用户银行卡，由于支付资金为运营垫付，无法走原路退款只能走付款，但这种交易类的付款必须要经过清结算，而清结算暂不支持明细级的实时清结算
		* 接口或消息消费
...
* 会临时调整清算天数，所以有重复出批次的 case，需要取消之前的批次
	...
	* 出批次时捞出的明细字段携带版本号，有效明细的版本号必须为空、当前批次（重入）、当日无效批次
...
难点：
* 全新的业务，系统处理为黑盒，只有输出的文件
	* 和重构的思路类似，做功能的平替，先保证自研系统能够输出和外采系统一致的文件
	* 文件的生成规则同产品、银行三方沟通
	* 在 diff 过程中不对修改逻辑
* 如何将原本外采系统融入进理财资金的清结算流程
	* 根据输出的文件内容和作用为基础，将外采系统这部分业务整体看作结算流程的一环：负责销售监管户资金的出款，这个定义可以和重构后的结算系统领域职责匹配
	* 报送的文件有三大类：
		* 交易明细：原始交易凭证，不参与资金流，是监管需要的交易信息
		* 资金入款：监管行用来和划入监管户的资金对账，可以看作结算通知
		* 资金出款：监控控制监管户资金的划出，相当于结算指令
	* 实现上
		* 还是对交易数据的清算、落地清算结果、结算使用清算结果划款
		* 只是划款的方式需要单独封装一层人工后台并以文件的形式产出
		* 底层数据在长期看是要能和其他清结算流程衔接或服用的，从而根据状态机能够 cover 一笔订单在 dxm 和基金公司的整个资金链路
	* 理想情况：
		* 上行：
			* 处理完申请后，划款指令和上行款的流程复用这部分数据
			* 再服用这部分数据做正向监管数据中资金入款的上报
			* 流程间用状态机控制资金位置
			* 正向确认数据用来做正向资金出款的上报
		* 下行：
			* 只关注确认和分红，解析后做逆向数据的入金和出金上报
			* 然后根据这些数据单独处理转购清算和下行清算
		* 可以看出上行和下行流程的资金链路是相反的
* 新老系统切换
	* 前置条件：提前支持多个批次的监管上报
	* TA 维度切换
	* 其他批次手动加入某支基金做小流量验证
...
需要注意的是，BLOCKED 阻塞是因为竞争不到锁资源而被迫停止，而 WAITING 和 TIME WAITING 则仅仅是自行产生阻塞，和锁无关。这三种阻塞仅仅定义在 Java 层面，而操作系统中只有一种阻塞状态

> OS 层面阻塞的本质：将进程挂起，不再参与进程调度，即 OS 调度器不会再将 CPU 时间片分给此进程

### start / run

* start() 是线程的启动方法，调用 native 的 start0()，start0() 会请求 OS 的 API 创建线程，等待调度
* run() 只是一个对象的普通方法，因为会在 start0() 中被调用所以线程才会执行它，如果直接调用就和其他方法一样只会顺序执行

> 需要注意的是，线程池底层都是使用了 run()，也就是说只要被分配了 CPU 资源就会立即执行 run 的逻辑
...
* 它会唤醒锁对象上的其他线程，让这些线程去重新竞争锁
 ...
 > 需要注意的是，notify 对线程的唤醒是指让它们从 OS 的阻塞态中恢复，变成「可被分配 CPU 资源」的状态

### sleep

sleep 不会释放锁，但会释放 CPU 资源

> 因为锁是由 JVM 实现的，而线程的阻塞是 JVM 借用了 OS 的实现，二者没有必然的联系
...
* ThreadLocal 持有 ThreadLocalMap 的引用，以 ThreadLocal 对象作为 key 进行值的存放
...
> 根据 createMap 可知，每个 Thread 对象都持有一个 ThreadLocalMap，因为一个线程可以有多个 TL，所以需要用 TL 做 key
...
可能导致内存泄漏的是线程在堆上创建的 TLM 对象中的 key、value
...
只要 Thread 对象一直存在（线程池），线程就一直持有 TLM 的引用，那么 TLM 中的 value 对象就一直被持有，无法被 GC
...
Spring 中 TL 的应用：
* 将 db 连接绑定到当前线程来保证这个线程中对 db 的操作用的是同一个连接，就是将 db 连接放在了 TL 中